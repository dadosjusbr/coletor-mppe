...
#Necessário devido a permutação de lugar das colunas: Indenizações - Contribuição Previdenciária
def employees_parser_2020(file_path):
    data = read(file_path)
    ...
    for row in rows:
        ...
        temp_remu = row[10] #Outras Remunerações Retroativas/Temporárias
        idemnity = row[11] #Indenizações
...
def employees_idemnity(file_path, employees):
    ...
    for row in rows:
        ...
        prm =  row[4] #0053-L PREM S/TRB
        
        transporte =  row[5] #0243-AUX TRANSP
        housing_aid = row[6] #0261-AUX MORADIA
        aux_refei = row[7] #0269-AUX REFEICAO
        aux_ali = row[8] #0270-AUX ALIM
        aux_saude = row[9] #0272-AUX SAUDE
        
        abon  = row[10] #0098-ABON PER ATR
        diff = row[11] #0201-DIF ENTRANC
        adc_ser_ex = row[12] #0244-ADC SERV EXT
        serv_ex_pĺt = row[13] #0251-SERV EXT PLT
        ind_ferias = row[14] #0265-IND FERIAS
        adc_at_esp = row[15] #0267-ADC AT ESPEC
        grt_sub = row[16] #0271-GRT SUB FGMP
        adc_exercicio = row[17] #0275-AD EXERCICIO
        adc_pr = row[18]  #0279-ADIC PR/TEMP
        atr_venci = row[19] #0400-VENCIM ATR
        diff_entr = row[20] #0401-DIF ENTR ATR
        quino  = row[21] #0403-QUINQ ATR
        fer_at = row[22] #0408-AB FER PR AT
        pens_ali = row[23] #0416-PENS AL ATR
        indz_ass = row[24] #0420-INDZ ASS ATR
        sal_atr = row[25] #0426-13 SAL ATR
        indz_coo = row[26] #0429-INDZ COO ATR
        ad_li = row[27] #0434-AD L16307 AT
        sub_atr = row[28] #0435-SUBSIDIO ATR
        aux_trp = row[29] #0443-AUX TRP ATR
        ad_sev_ex = row[30] #0444-AD SV EX ATR
        ad_sev_pl = row[31] #0451-SV EX PL ATR
        ad_com = row[32] #0459-COM LIC ATR
        adc_ferias = row[33] #0463-A FERIAS ATR
        ad_at = row[34] #0467-AD AT ES ATR
        aux_ref_atr = row[35] #0469-AUX REF ATR
        aux_ali_atr = row[36] #0470-AUX ALIM ATR
        gr_su = row[37] #0471-GR SU FG ATR
        aux_saude_atr = row[38] #0472-AUX SAUDE AT
        adc_exe = row[39] #0475-ADIC EXE ATR
        adc_pr = row[40] #0479-AD PR/TP ATR
        grt_fgmp = row[41] #0480-GRT FGMP ATR
        in_cgsmp = row[42] #0483-IN CGSMP ATR
        ind_ouv = row[43] #0486-IND OUV ATR
        ats_tribu = row[44] #522 ATS.TRIBUTAV
        ats_ntribut = row[45] #523 ATS.N.TRIBUT
        pae_ntrb = row[46] #558 PAE NTRB ATR
        pae_trib = row[47] #559 PAE TRIB ATR
    ...
    for file_name in file_names:
        if 'vi' not in file_name:
            if int(year) <= 2019:
                ...
                employees.update(employees_parser_2020(file_name))
